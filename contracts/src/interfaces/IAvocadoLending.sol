...
interface IAvocadoLending {
    ...
    struct Borrower {
        ...
        uint256 accruedInterest;   // ALG-04M fix: Accumulated unpaid interest
        uint256 agreedBorrowRateBps; // ALG-03M fix: Per-borrower locked-in borrow rate
    ...
    struct LoanTerms {
        uint256 amount;            // Remaining principal for this loan
    ...
    error BorrowRateExceeded(uint256 currentRate, uint256 maxRate);
    error BorrowRateChanged(uint256 currentRate, uint256 agreedRate);
    ...
    /// @param maxBorrowRateBps ALG-01M fix: Maximum borrow rate the borrower is willing to accept
    function borrow(uint256 amount, uint256 maxBorrowRateBps) external returns (uint256 loanId);
    ...
    /// @notice Accrue interest for specific borrowers
    /// @param borrowers ALG-02M fix: Array of borrower addresses to accrue (replaces full-list iteration)
